...
import cz.etnetera.seb.listener.impl.LoggingListener;
import cz.etnetera.seb.listener.impl.PageSourceListener;
import cz.etnetera.seb.listener.impl.ScreenshotListener;
import cz.etnetera.seb.listener.impl.SebLogListener;
import cz.etnetera.seb.listener.impl.WebDriverLogListener;
import cz.etnetera.seb.source.ChainedPropertiesSource;
import cz.etnetera.seb.source.DataSource;

/**
 * Configures Seb. You are free to add whatever methods you want as you can
 * easily get overridden configuration
 * {@link SebContext#getConfiguration(Class)}.
 * 
 * It allows to load properties from resource, path, file or {@link Properties}
 * directly and it has higher priority than default values. You can obtain them
 * later using {@link SebContext#getProperty(String)} and similar property based
 * methods.
 * 
 * Tries to get all possible values from chained properties. Properties are
 * prefixed with 'seb.':
 * 
 * baseUrlProperty = seb.baseUrlProperty baseUrlRegexProperty =
 * seb.baseUrlRegexProperty urlVerificationProperty =
 * seb.urlVerificationProperty waitTimeoutProperty = seb.waitTimeoutProperty
 * waitRetryIntervalProperty = seb.waitRetryIntervalProperty reportedProperty =
 * seb.reportedProperty reportDirProperty = seb.reportDirProperty
 * 
 * For now only {@link BasicSebConfiguration#getDriver(DesiredCapabilities)},
 * {@link BasicSebConfiguration#getCapabilities()} and
 * {@link BasicSebConfiguration#getListeners()} are not supported methods and
 * calls default methods directly.
 * 
 * In addition you can add any data using
 * {@link BasicSebConfiguration#addData(String, Object)} and obtain it using
 * {@link SebContext#getData(String, Class)}.
 */
public class BasicSebConfiguration implements SebConfiguration, ChainedPropertiesSource, DataSource {
	...
	/**
	 * Default base URL for pages. Override this for different value.
	 * 
	 * @return The base URL
	 */
	...
	/**
	 * Default hub URL for RemoteWebDriver. Override this for different value.
	 * 
	 * @return The hub URL
	 */
	...
	/**
	 * Returns default {@link WebDriver} instance. Override this for different
	 * value.
	 * 
	 * @param caps
	...
	/**
	 * Returns optional default {@link DesiredCapabilities}. They are
	 * constructed from system properties starting with
	 * {@link BasicSebConfiguration#CAPABILITIES_PREFIX}. Override this for
	 * different value.
	 * 
	...
	protected DesiredCapabilities getDefaultCapabilities() {
		...
		getMergedProperties(
				entry -> entry.getKey() != null && entry.getKey().toString().startsWith(CAPABILITIES_PREFIX))
						.forEach((key, value) -> {
	...
	/**
	 * Returns default wait timeout. Override this for different value.
	 * 
	...
	/**
	 * Returns default wait retry interval. Override this for different value.
	 * 
	 ...
	 * {@link BasicSebConfiguration#DEFAULT_PROPERTIES_RESOURCE_NAME} and adds
	 * them at the end of actual properties list.
	 * 
	 ...
	 * Loads properties from resource
	 * {@link BasicSebConfiguration#DEFAULT_PROPERTIES_RESOURCE_NAME} and adds
	 * them before specific key in actual properties list.
	 * 
	 * @param beforeKey
	...
	public BasicSebConfiguration addDefaultPropertiesBefore(String beforeKey) {
		return (BasicSebConfiguration) addResourcePropertiesBefore(beforeKey, DEFAULT_PROPERTIES_KEY,
	 ...
	 * Loads properties from resource
	 * {@link BasicSebConfiguration#DEFAULT_PROPERTIES_RESOURCE_NAME} and adds
	 * them after specific key in actual properties list.
	 * 
	 * @param afterKey
	...
	public BasicSebConfiguration addDefaultPropertiesAfter(String afterKey) {
		return (BasicSebConfiguration) addResourcePropertiesAfter(afterKey, DEFAULT_PROPERTIES_KEY,
				DEFAULT_PROPERTIES_RESOURCE_NAME);
	}
	...
	/**
	 * Adds {@link System#getProperties()} before specific key in actual
	 * properties list.
	 * 
	 * @param beforeKey
	 *            The before properties key
	...
	/**
	 * Adds {@link System#getProperties()} after specific key in actual
	 * properties list.
	 * 
	 * @param afterKey
	 *            The after properties key
	...
	/**
	 * Loads properties from resource
	 * {@link BasicSebConfiguration#DEFAULT_PROPERTIES_RESOURCE_NAME} and pushes
	 * them at the start of actual properties list.
	 * 
	 * @return Same instance
	 */
	...
	/**
	 * Pushes {@link System#getProperties()} at the start of actual properties
	 * list.
	 * 
	 * @return Same instance
	 */
